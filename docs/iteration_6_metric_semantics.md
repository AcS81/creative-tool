...
- `silence_for_emphasis_fidelity` — Score + diagnostic list `{ start, end, alignedBeat?, alignedPunchline? }`. Unit: count/duration of silence spans (>0.6s). Interpretation: high when silences coincide with hooks/payoffs or resets; low when absent or mistimed. Tooltip: “Silence used intentionally for emphasis.”
...
- `timing_score` — Combines `time_to_hook_seconds`, `hook_strength_score`, `beats_vs_edits_alignment`, `silence_for_emphasis_fidelity`, and punch timing. Interpretation: high=beats, cuts, silences, and punches land at effective times. Tooltip: “Timing of hooks, edits, and emphases.”
